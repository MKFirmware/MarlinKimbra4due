...
#include "base.h"
  ...
  void GFX::_pixel2d_clipped(uint16_t color, const struct point *pt) {
    int x, y;
    ...
    drawPixel(x + _left, y + _top, color);
  }
  ...
  void GFX::line_to(int ndx, const float *pos) {
    ...
    if (ndx >= 0 && ndx < VC_MAX) {
      float c1[3], c2[3];
      fcolor(c1, _color[ndx], _cursor.position[Y_AXIS], _max[Y_AXIS]);
      fcolor(c2, _color[ndx], pos[Y_AXIS], _max[Y_AXIS]);
  ...
  void GFX::pixel_at(int ndx, const float *pos) {
    struct point loc;

    if (ndx < 0 || ndx >= VC_MAX) return;
